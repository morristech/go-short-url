...
BIN := go-short-url-server
SRC_BIN := server

# This repo's root import path (under GOPATH).
PKG := github.com/me-io/go-short-url
...
test-docker: build-dirs
	...
	@docker container rm go-short-url-server-redis -f > /dev/null 2>&1 || true
	@docker run                                                                   \
	    -ti                                                                       \
	    --rm 	     														      \
	    --name go-short-url-server-redis 											  \
	    -d redis:alpine redis-server --appendonly yes
	@docker run                                                                   \
	    -e "REDIS_URL=redis://redis:6379"										  \
	    --link go-short-url-server-redis:redis                                         \
	    -ti                                                                       \
	    --rm                                                                      \
	    -u $$(id -u):$$(id -g)                                                    \
	    -v "$$(pwd)/.go:/go"                                                      \
	    ...
	    " ; 																	  \
		docker container rm go-short-url-server-redis -f > /dev/null 2>&1
